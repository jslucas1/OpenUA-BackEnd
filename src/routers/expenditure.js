...
router.get('/expenditures/:payee/:amount/:transactionNumber/:poNumber/:checkNumber/:agency/:funding/:category/:startDate/:endDate', async (req, res) => {
    try {
        ...
        const myExpenditures = await Expenditure.find({ "PAYEE": payee, "TRANS_AMT": amount, "TRAN_NO": transactionNumber, "PO_NO": poNumber, "CHECK_NO": checkNumber, "AGENCY": agency, "FUNDING": funding, "CATEGORY": category, $and: [{ "DATE": { $gte: startDate } }, { "DATE": { $lte: endDate } }] }).collation({ locale: "en", strength: 2 })
